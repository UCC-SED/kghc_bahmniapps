...
angular.module('authentication')
        ...
        this.loginUser = function(username, password, location, otp) {
            ...
            createSession(username, password, otp).then(function(data) {
                if (data.authenticated) {
                    $bahmniCookieStore.put(Bahmni.Common.Constants.currentUser, username, {
                        path: '/',
                        expires: 7
                    });
            ...
            userService.getUser(currentUser).then(function(data) {
                userService.getProviderForUser(data.results[0].uuid).then(function(providers) {
                        if (!_.isEmpty(providers.results) && hasAnyActiveProvider(providers.results)) {
                            ...
                            console.log($bahmniCookieStore.get(Bahmni.Common.Constants.locationCookieName).name);
    ...
    }]).directive('logOut', ['sessionService', '$window', 'configurationService', 'auditLogService', '$bahmniCookieStore', '$http', '$rootScope', 'spinner', function(sessionService, $window, configurationService, auditLogService, $bahmniCookieStore, $http, $rootScope, spinner) {
        return {
                ...
                element.bind('click', function() {
                    scope.$apply(function() {
                        ...
                        return $http.get(url, {
                            ...
                            var url = Bahmni.Common.Constants.gothomisLogOut;
                            spinner.forPromise($http.get(url, {
                                params: {
