...
angular.module('bahmni.clinical').controller('ConsultationController', ['$scope', '$rootScope', '$state', '$location', 'clinicalAppConfigService', 'diagnosisService', 'urlHelper', 'contextChangeHandler',
    ...
    function ($scope, $rootScope, $state, $location, clinicalAppConfigService, diagnosisService, urlHelper, contextChangeHandler,
        ...
        var submitDispositionToGothomis = function (providerUuid, patientUuid, dispositionNotes, locationUuid) {
            spinner.forPromise(integrationService.submitDisposition(providerUuid,
                patientUuid,
                dispositionNotes, locationUuid
            ).then(function (response) {
            }));
        };
                                    ...
                                    if ($scope.gothomisIntegrationStatus) {
                                        ...
                                        if (encounterData.disposition != null) {
                                            if (encounterData.disposition.code == "ADMIT") {
                                                submitDispositionToGothomis(currentProviderUuid, encounterData.patientUuid, encounterData.disposition.additionalObs[0].value, encounterData.locationUuid);
                                            }
                                        }
                                    }
